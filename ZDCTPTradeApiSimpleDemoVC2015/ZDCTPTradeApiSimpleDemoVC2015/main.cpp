...
void MarketTest()
{
	...
	int err = -1;
#ifdef _DEBUG
	market = XFinApi_CreateMarketApi("XTA_W32/Api/ZDCTP_v20171130/XFinApi.ZDCTPTradeApid.dll", &err);
#else
	market = XFinApi_CreateMarketApi("XTA_W32/Api/ZDCTP_v20171130/XFinApi.ZDCTPTradeApi.dll", &err);
#endif
	...
	openParams.Configs.insert(std::make_pair(std::string("LoginAddress"),Cfg.TradeAddress));
...
void TradeTest()
{
	...
	int err = -1;
#ifdef _DEBUG
	trade = XFinApi_CreateTradeApi("XTA_W32/Api/ZDCTP_v20171130/XFinApi.ZDCTPTradeApid.dll", &err);
#else
	trade = XFinApi_CreateTradeApi("XTA_W32/Api/ZDCTP_v20171130/XFinApi.ZDCTPTradeApi.dll", &err);
#endif
	if (err || !trade)
	{
	...
	openParams.UserID = Cfg.UserName;
	openParams.Password = Cfg.Password;
	openParams.Configs.insert(std::make_pair(std::string("CurrencyNo"), Cfg.CurrencyNo));//设置币种，底层根据币种选择默认资金账户
	openParams.IsUTF8 = false;
