...
- ZDCTPTradeApi : 是基于ShZdCtpDll 的一个统一封装，可实现多语言的使用，简化开发过程。
- QQ群：650920857
- 官方网址 :  www.xfinapi.com

#### 软件架构
- ITradeApi ： 统一接口，位于Cpp目录下，使用者可直接引用此目录的头文件和lib进行开发，ZDCTPTradeApi .dll会动态加载到程序中，不进行依赖。
- ZDCTPTradeApi .dll : 对底层ShZdFutureMarketApi、ShZdFutureTradeApi的封装。
